...
const MAX_SCAN_INTERVAL_HOURS = 168; // one week upper guardrail
...
export default class PluginHealthMonitor extends Plugin {
  ...
  async saveSettings(): Promise<void> {
    this.settings.autoScanIntervalHours = this.clampInterval(this.settings.autoScanIntervalHours);
  ...
  async runScan(showNotice: boolean): Promise<void> {
    if (this.isScanning) {
      if (showNotice) {
        new Notice('A plugin health scan is already running.');
      }
      return;
    }
  ...
  clampInterval(hours: number): number {
    if (Number.isNaN(hours) || hours <= 0) {
      return DEFAULT_SETTINGS.autoScanIntervalHours;
    }
    return Math.min(Math.max(hours, MIN_SCAN_INTERVAL_HOURS), MAX_SCAN_INTERVAL_HOURS);
  }
}
      ...
      .addText((text) =>
        text
          ...
          .onChange(async (value) => {
            ...
            const clamped = this.plugin.clampInterval(parsed);
            this.plugin.settings.autoScanIntervalHours = clamped;
            await this.plugin.saveSettings();
